...
;; ossekai 用に com-junkawasaki/root superproject の manifest/edn-datomize.bb
;; から移植（Phase 3 fan-out, gftdcojp/net-kotobase の wrap-generic 拡張版を経由）。
;; schema-path をこの repo のルート schema.edn に向けている以外はロジック同一
;; ＋ wrap-generic モード（既に名前空間付きキーを持つ map 用。ossekai の
;; cells/*.edn は :cell/id 等、既に idiomatic に名前空間付きなのでこちらを使う）。
;;
...
;; ossekai (child repo) has no manifest/ dir — schema lives at repo root.
